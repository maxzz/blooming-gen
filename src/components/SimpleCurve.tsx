...
function parsePathString(pathString: string) {
    ...
    const tCommand = new RegExp("([rstm])[" + spaces + ",]*((-?\\d*\\.?\\d*(?:e[\\-+]?\\d+)?[" + spaces + "]*,?[" + spaces + "]*)+)", "ig");
    const pathValues = new RegExp("(-?\\d*\\.?\\d*(?:e[\\-+]?\\d+)?)[" + spaces + "]*,?[" + spaces + "]*", "ig");

    pathString.replace(pathCommand, 
...
/*
'src M18,69.48s-.6-11.27-3-30.86S30.43.34,30.43.34'
(3) [Array(3), Array(5), Array(5)]
    0: (3) ["M", 18, 69.48]
    1: (5) ["s", -0.6, -11.27, -3, -30.86]
    2: (5) ["S", 30.43, 0.34, 30.43, 0.34]
*/
